...
// @require        https://greasyfork.org/scripts/380695-%C3%BCbersetzung/code/%C3%9Cbersetzung.js?version=863404
...
let serversDatas = {
    ...
    "de":{
        ...
        "ageDates"            : [26402796, 26446632, 26490408, 26534184, 26579400, 26621736, 26665512, 26709288, 26753004, 26796780,26840676,26884452,26928228,26972004,27015780,27059556,27103332,27147108,27190884,27234660,27278436,27322212]
    },
    "at":{
        "flag"                : "AUT",
        "language"            : "German",
...
Game.pages.add('transfermarket',{
    ...
    $('#club').after('<img style="float:right;margin:2px 2px 0 0;cursor:pointer;" id="clearText" src="'+Tool.sources.getLink('remove','png')+'" alt="remove" width="10px">');
...
let Tool = new (class{
    ...
    createToolMenu(){
        ...
        $('#section-inner-container').after(
            ...
            `                  <img id="toggleExtraSettings" src="${Tool.sources.getLink('show','png')}" alt="show" width="15px" style="cursor:pointer;margin-top:2px;">`+
            `               </p>`+
            `            </td>`+
            `         </tr>`+
            `      </tfoot>`+
    ...
    createMenuEvents(){
        ...
        $('#toggleExtraSettings').click(function(){
            ...
            $(this).attr({
                'src': Tool.sources.getLink(a,'png'),
                'alt': a
            });
            ...
            onload: function(response) {
                ...
                if(version!=currentVersion){
                    ...
                    div.content =
                        ...
                        `   ${GetText('UpdateTheScriptInfo', {args:['<a href="https://greasyfork.org/scripts/40715-fcup-script/code/FCup%20Script.user.js" style="font-size:14px">','</a>']})}`+
                        `</p>`+
...
Tool.features.add('ConstructionCountdown','main',function(){
    ...
    $('#clubinfocard > ul').append(
        `<li>`+
        `   <img width="16px" src="${Tool.sources.getLink('construction','png')}" alt="construction" style="margin:-3px 0 0 0; vertical-align:middle;">`+
...
Tool.features.add('RematchMatch','main',function(){
    ...
    for(i = 0; i < oa.length ; i++){
        ...
        if(!find){
            let ul = oa[i];
            ul.find('.show-button').append(`<img class="sendSimulation" k="${clubId}" src="${Tool.sources.getLink('again','png')}" alt="again" style="cursor:pointer; vertical-align:middle;" width="35px">`);
        }
    }
    ...
    $('#players-table-skills > tbody > tr').each(function(){
        ...
        for(let i = 0 ; i < developSkills.length ; i++){//Geliştirilecek yetenek aranıyor
            ...
            if(s.hasClass("maximum")) continue;//Bu geliştirilecek yetenek maksimumsa bir sonraki geliştirilecek yeteneğe bak.
                ...
                s.html(
                    `<img title="${GetText('ImproveSkillTitle', {tag:0, args:[skillName]})}!!!" tool_tt="ImproveSkillTitle_${encodeURIComponent(JSON.stringify([skillName]))}" src="${Tool.sources.getLink('here','gif')}" alt="here" width="15px" height="15px" style="-webkit-transform:rotate(90deg); margin:-8px 0 0 ${left}px; position:absolute;">`+
                    value
                );
        ...
        addImage : function(playerData,table,index){
            ...
            $(tds).each(function(){
                ...
                $(this).html(
                    ...
                    `   <img title="${GetText('ClubExchange', {tag:0, args:args})}" tool_tt="ClubExchange_${encodeURIComponent(JSON.stringify(args))}" src="${Tool.sources.getLink('exchange','png')}" alt="exchange" height="15px" style="background-color:#00fff7; border-radius:50%; cursor:pointer; margin-right:4px;">`+
                    `</a>`+
    ...
    if(data.length){/*Show*/
        ...
        for(let i = 0 ; i < data.length ;i++){
            ...
            text+=
                ...
                `   <td>`+
                `      <img src="${Tool.sources.getLink('clock','png')}" alt="clock" style="margin:0 2px 1px 0;cursor:help;text-align:left;" title="${GetText('ServerDate', {tag:0})}" tool_tt="ServerDate">`+
...
Tool.features.add('ShowBoughtPlayers','transfermarket',function(){
    ...
    function start(BoughtPlayers){
        ...
        $(`<img style="float:right;cursor:pointer;margin-Right:5px;" src="${Tool.sources.getLink('download','png')}" alt="download" width="15px" height="15px" title="${GetText('Download', {tag:0})}" tool_tt="Download">`)
            .appendTo(h3)
            .click(function(){ DownloadAsTextFile(JSON.stringify(BoughtPlayers,null,'\t'), "Bought Players Datas"); });
 
        $(`<img style="margin-Right:7px;float:right;cursor:pointer" src="${Tool.sources.getLink('remove2','png')}" alt="remove2" width="15px" height="15px" title="${GetText('Delete', {tag:0})}" tool_tt="Delete">`)
            .appendTo(h3)
            .click(function(){
        ...
        BoughtPlayers.forEach((a,i)=>{
            tbody.append(
                ...
                `      <img class="DeletePurchasedPlayerData" playerid="${a.playerId}" src="${Tool.sources.getLink('remove3','png')}" alt="remove3" heigth="15px" width="15px" style="cursor:pointer">`+
                `   </td>`+
                `</tr>`
            );
        });
...
Tool.features.add('DownloadTable',['league','statistics','squadstrenght','goalgetter'],function(){
    ...
    $(`<img src="${Tool.sources.getLink('printscreen','png')}" alt="printscreen" height="30px" style="cursor:pointer;" title="${GetText('DownloadTable', {tag:0})}" tool_tt="DownloadTable">`)
...
Tool.features.add('AddImage',['press->article','club_profile'],function(){
    ...
    $(`<li class="" title="${GetText('AddImage', {tag:0})}" tool_tt="AddImage" style="cursor:pointer;" onmouseenter="$(this).addClass('hover')" onmouseleave="$(this).removeClass('hover')">`+
      `   <img src="${Tool.sources.getLink('image','png')}" alt="image" width="20px" height="20px">`+
      `</li>`
